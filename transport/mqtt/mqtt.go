...
import (
	...
	"github.com/edgehook/ithings/common/crypto"
)
...
type Client struct {
	...
	OnReconnectFn func(*Client, *paho.ClientOptions)
}
...
func NewMQTTClient(host, clientID, usr, pwd string,
	...
	options.SetPassword(string(pwd))
...
func (c *Client) Publish(topic, payload string, qos byte, retained bool) error {
	// klog.Infof("MessagePublish topic: %s, payload: %s", topic, payload)
	if topic == "" {
	...
	//publish the message
	c.mutex.Lock()
	token := c.client.Publish(topic, qos, retained, crypted)
	c.mutex.Unlock()
...
func (c *Client) Subscribe(topic string, qos byte, fn func(tpc string, payload []byte)) error {
	...
	callback := func(client paho.Client, message paho.Message) {
		...
		if err != nil {
			klog.Errorf("error message format, Ignored!")
			return
		}
